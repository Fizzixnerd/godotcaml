module GlobalEnum0 = struct
  ...
  module Side = struct
    ...
    *)
    ...
    (** Right side, usually used for \[Control\] or \[StyleBox\]-derived
        classes. *)
    let _SIDE_RIGHT = 2

    (** Bottom side, usually used for \[Control\] or \[StyleBox\]-derived
        classes. *)
    let _SIDE_BOTTOM = 3
  end
  ...
  module Orientation = struct
    ...
    (** General vertical alignment, usually used for \[Separator\],
        \[ScrollBar\], \[Slider\], etc. *)
    let _VERTICAL = 1

    (** General horizontal alignment, usually used for \[Separator\],
        \[ScrollBar\], \[Slider\], etc. *)
    let _HORIZONTAL = 0
  end
  ...
  module ClockDirection = struct
    ...
    (** Clockwise rotation. Used by some methods (e.g. \[method
        Image.rotate_90\]). *)
    let _CLOCKWISE = 0
  ...
  module InlineAlignment = struct
    ...
    (** Aligns the top of the inline object (e.g. image, table) to the position
        of the text specified by \[code\]INLINE_ALIGNMENT_TO_ * \[/code\]
        constant. *)
    let _INLINE_ALIGNMENT_TOP_TO = 0

    (** Aligns the center of the inline object (e.g. image, table) to the
        position of the text specified by \[code\]INLINE_ALIGNMENT_TO_ *
        \[/code\] constant. *)
    let _INLINE_ALIGNMENT_CENTER_TO = 1

    (** Aligns the baseline (user defined) of the inline object (e.g. image,
        table) to the position of the text specified by
        \[code\]INLINE_ALIGNMENT_TO_ * \[/code\] constant. *)
    ...
    (** Aligns the position of the inline object (e.g. image, table) specified
        by \[code\]INLINE_ALIGNMENT_ * _TO\[/code\] constant to the top of the
        text. *)
    let _INLINE_ALIGNMENT_TO_TOP = 0

    (** Aligns the position of the inline object (e.g. image, table) specified
        by \[code\]INLINE_ALIGNMENT_ * _TO\[/code\] constant to the center of
        the text. *)
    let _INLINE_ALIGNMENT_TO_CENTER = 4

    (** Aligns the position of the inline object (e.g. image, table) specified
        by \[code\]INLINE_ALIGNMENT_ * _TO\[/code\] constant to the baseline of
        the text. *)
    ...
    (** Aligns top of the inline object (e.g. image, table) to the top of the
        text. Equivalent to \[code\]INLINE_ALIGNMENT_TOP_TO |
    ...
    (** Aligns center of the inline object (e.g. image, table) to the center of
        the text. Equivalent to \[code\]INLINE_ALIGNMENT_CENTER_TO |
    ...
    (** Aligns bottom of the inline object (e.g. image, table) to the bottom of
        the text. Equivalent to \[code\]INLINE_ALIGNMENT_BOTTOM_TO |
        INLINE_ALIGNMENT_TO_BOTTOM\[/code\]. *)
    let _INLINE_ALIGNMENT_BOTTOM = 14

    (** A bit mask for \[code\]INLINE_ALIGNMENT_ * _TO\[/code\] alignment
        constants. *)
    let _INLINE_ALIGNMENT_IMAGE_MASK = 3

    (** A bit mask for \[code\]INLINE_ALIGNMENT_TO_ * \[/code\] alignment
        constants. *)
  ...
  module Key = struct
    ...
    (** Enum value which doesn''t correspond to any key. This is used to
        initialize \[enum Key\] properties with a generic state. *)
    let _KEY_NONE = 0
  ...
  module KeyModifierMask = struct
    ...
    (** Modifier key mask. *)
    let _KEY_MODIFIER_MASK = 2130706432

    (** Automatically remapped to \[constant KEY_META\] on macOS and \[constant
        KEY_CTRL\] on other platforms, this mask is never set in the actual
  ...
  module KeyLocation = struct
    ...
    (** Used for keys which only appear once, or when a comparison doesn''t need
        to differentiate the \[code\]LEFT\[/code\] and \[code\]RIGHT\[/code\]
        versions. For example, when using \[method InputEvent.is_match\], an
        event which has \[constant KEY_LOCATION_UNSPECIFIED\] will match any
        \[enum KeyLocation\] on the passed event. *)
    let _KEY_LOCATION_UNSPECIFIED = 0

    (** A key which is to the left of its twin. *)
    let _KEY_LOCATION_LEFT = 1

    (** A key which is to the right of its twin. *)
    let _KEY_LOCATION_RIGHT = 2
  end
  ...
  module MouseButton = struct
    ...
    (** Enum value which doesn''t correspond to any mouse button. This is used
        to initialize \[enum MouseButton\] properties with a generic state. *)
  ...
  module MIDIMessage = struct
    ...
    (** MIDI message sent when a note is released. \[b\]Note:\[/b\] Not all MIDI
        devices send this message; some may send \[constant
        MIDI_MESSAGE_NOTE_ON\] with \[member InputEventMIDI.velocity\] set to
        \[code\]0\[/code\]. *)
    ...
    (** MIDI system exclusive (SysEx) message. This type of message is not
        ...
        \[b\]Note:\[/b\] Getting this message''s data from \[InputEventMIDI\] is
        not implemented. *)
    ...
    (** MIDI message sent to jump onto a new position in the current sequence or
        song. \[b\]Note:\[/b\] Getting this message''s data from
        \[InputEventMIDI\] is not implemented. *)
    let _MIDI_MESSAGE_SONG_POSITION_POINTER = 242

    (** MIDI message sent to select a sequence or song to play. \[b\]Note:\[/b\]
        Getting this message''s data from \[InputEventMIDI\] is not implemented.
    *)
    ...
    (** MIDI message sent 24 times after \[constant
        MIDI_MESSAGE_QUARTER_FRAME\], to keep connected MIDI devices
        synchronized. *)
  ...
  module Error = struct
    ...
    (** Methods that return \[enum Error\] return \[constant OK\] when no error
        occurred. Since \[constant OK\] has value \[code\]0\[/code\], and all
        other error constants are positive integers, it can also be used in
        boolean checks. \[codeblock\] var error = method_that_returns_error() if
        error != OK: printerr(""Failure!"")

        # Or, alternatively: if error: printerr(""Still failing!"")
        \[/codeblock\] \[b\]Note:\[/b\] Many functions do not return an error
        code, but will print error messages to standard output. *)
    let _OK = 0
    ...
    (** Help error. Used internally when passing \[code\]--version\[/code\] or
        \[code\]--help\[/code\] as executable options. *)
    let _ERR_HELP = 46

    (** Bug error, caused by an implementation issue in the method.
        \[b\]Note:\[/b\] If a built-in method returns this code, please open an
        issue on \[url=https://github.com/godotengine/godot/issues\]the GitHub
        Issue Tracker\[/url\]. *)
    let _ERR_BUG = 47
  ...
  module PropertyHint = struct
    ...
    (** Hints that an \[int\] or \[float\] property should be within a range
        specified via the hint string \[code\]""min,max""\[/code\] or
        \[code\]""min,max,step""\[/code\]. The hint string can optionally
        include \[code\]""or_greater""\[/code\] and/or
        \[code\]""or_less""\[/code\] to allow manual input going respectively
        above the max or below the min values. \[b\]Example:\[/b\]
        \[code\]""-360,360,1,or_greater,or_less""\[/code\]. Additionally, other
        keywords can be included: \[code\]""exp""\[/code\] for exponential range
        editing, \[code\]""radians_as_degrees""\[/code\] for editing radian
        angles in degrees (the range values are also in degrees),
        \[code\]""degrees""\[/code\] to hint at an angle and
        \[code\]""hide_slider""\[/code\] to hide the slider. *)
    let _PROPERTY_HINT_RANGE = 1

    (** Hints that an \[int\] or \[String\] property is an enumerated value to
        pick in a list specified via a hint string. The hint string is a comma
        separated list of names such as
        \[code\]""Hello,Something,Else""\[/code\]. Whitespaces are
        \[b\]not\[/b\] removed from either end of a name. For integer
        properties, the first name in the list has value 0, the next 1, and so
        on. Explicit values can also be specified by appending
        \[code\]:integer\[/code\] to the name, e.g.
        \[code\]""Zero,One,Three:3,Four,Six:6""\[/code\]. *)
    let _PROPERTY_HINT_ENUM = 2

    (** Hints that a \[String\] property can be an enumerated value to pick in a
        list specified via a hint string such as
        \[code\]""Hello,Something,Else""\[/code\]. Unlike \[constant
        PROPERTY_HINT_ENUM\], a property with this hint still accepts arbitrary
        values and can be empty. The list of values serves to suggest possible
        values. *)
    let _PROPERTY_HINT_ENUM_SUGGESTION = 3

    (** Hints that a \[float\] property should be edited via an exponential
        easing function. The hint string can include
        \[code\]""attenuation""\[/code\] to flip the curve horizontally and/or
        \[code\]""positive_only""\[/code\] to exclude in/out easing and limit
    ...
    (** Hints that a vector property should allow its components to be linked.
        For example, this allows \[member Vector2.x\] and \[member Vector2.y\]
        to be edited together. *)
    let _PROPERTY_HINT_LINK = 5

    (** Hints that an \[int\] property is a bitmask with named bit flags. The
        hint string is a comma separated list of names such as
        \[code\]""Bit0,Bit1,Bit2,Bit3""\[/code\]. Whitespaces are \[b\]not\[/b\]
        removed from either end of a name. The first name in the list has value
        1, the next 2, then 4, 8, 16 and so on. Explicit values can also be
        specified by appending \[code\]:integer\[/code\] to the name, e.g.
        \[code\]""A:4,B:8,C:16""\[/code\]. You can also combine several flags
        (\[code\]""A:4,B:8,AB:12,C:16""\[/code\]). \[b\]Note:\[/b\] A flag value
        must be at least \[code\]1\[/code\] and at most \[code\]2 * * 32 -
        1\[/code\]. \[b\]Note:\[/b\] Unlike \[constant PROPERTY_HINT_ENUM\], the
        previous explicit value is not taken into account. For the hint
        \[code\]""A:16,B,C""\[/code\], A is 16, B is 2, C is 4. *)
    let _PROPERTY_HINT_FLAGS = 6

    (** Hints that an \[int\] property is a bitmask using the optionally named
        2D render layers. *)
    ...
    (** Hints that a \[String\] property is a path to a file. Editing it will
        show a file dialog for picking the path. The hint string can be a set of
        filters with wildcards like \[code\]"" * .png, * .jpg""\[/code\]. *)
    let _PROPERTY_HINT_FILE = 13

    (** Hints that a \[String\] property is a path to a directory. Editing it
        will show a file dialog for picking the path. *)
    let _PROPERTY_HINT_DIR = 14

    (** Hints that a \[String\] property is an absolute path to a file outside
        the project folder. Editing it will show a file dialog for picking the
        path. The hint string can be a set of filters with wildcards, like
        \[code\]"" * .png, * .jpg""\[/code\]. *)
    let _PROPERTY_HINT_GLOBAL_FILE = 15

    (** Hints that a \[String\] property is an absolute path to a directory
        outside the project folder. Editing it will show a file dialog for
        picking the path. *)
    let _PROPERTY_HINT_GLOBAL_DIR = 16

    (** Hints that a property is an instance of a \[Resource\]-derived type,
        optionally specified via the hint string (e.g.
        \[code\]""Texture2D""\[/code\]). Editing it will show a popup menu of
        valid resource types to instantiate. *)
    let _PROPERTY_HINT_RESOURCE_TYPE = 17

    (** Hints that a \[String\] property is text with line breaks. Editing it
        will show a text input field where line breaks can be typed. *)
    let _PROPERTY_HINT_MULTILINE_TEXT = 18

    (** Hints that a \[String\] property is an \[Expression\]. *)
    let _PROPERTY_HINT_EXPRESSION = 19

    (** Hints that a \[String\] property should show a placeholder text on its
        input field, if empty. The hint string is the placeholder text to use.
    *)
    ...
    (** If a property is \[String\], hints that the property represents a
        particular type (class). This allows to select a type from the create
        dialog. The property will store the selected type as a string. If a
        property is \[Array\], hints the editor how to show elements. The
        \[code\]hint_string\[/code\] must encode nested types using
        \[code\]"":""\[/code\] and \[code\]""/""\[/code\]. \[codeblocks\]
        \[gdscript\] # Array of elem_type. hint_string = ""%d:"" % \[elem_type\]
        ...
        elem_type, elem_hint, elem_hint_string\] \[/gdscript\] \[csharp\] //
        Array of elemType. hintString = $""\{!elemType:D\}:""; hintString =
        $""\{!elemType:\}/\{!elemHint:D\}:\{!elemHintString\}""; //
        Two-dimensional array of elemType (array of arrays of elemType).
        hintString = $""\{!Variant.Type.Array:D\}:\{!elemType:D\}:""; hintString
        =
        ...
        \[/csharp\] \[/codeblocks\] \[b\]Examples:\[/b\] \[codeblocks\]
        \[gdscript\] hint_string = ""%d:"" % \[TYPE_INT\] # Array of integers.
        hint_string = ""%d/%d:1,10,1"" % \[TYPE_INT, PROPERTY_HINT_RANGE\] #
        Array of integers (in range from 1 to 10). hint_string =
        ""%d/%d:Zero,One,Two"" % \[TYPE_INT, PROPERTY_HINT_ENUM\] # Array of
        integers (an enum). hint_string = ""%d/%d:Zero,One,Three:3,Six:6"" %
        \[TYPE_INT, PROPERTY_HINT_ENUM\] # Array of integers (an enum).
        hint_string = ""%d/%d: * .png"" % \[TYPE_STRING, PROPERTY_HINT_FILE\] #
        Array of strings (file paths). hint_string = ""%d/%d:Texture2D"" %
        \[TYPE_OBJECT, PROPERTY_HINT_RESOURCE_TYPE\] # Array of textures.
        ...
        Two-dimensional array of textures. \[/gdscript\] \[csharp\] hintString =
        $""\{!Variant.Type.Int:D\}/\{!PropertyHint.Range:D\}:1,10,1""; // Array
        of integers (in range from 1 to 10). hintString =
        $""\{!Variant.Type.Int:D\}/\{!PropertyHint.Enum:D\}:Zero,One,Two""; //
        Array of integers (an enum). hintString =
        $""\{!Variant.Type.Int:D\}/\{!PropertyHint.Enum:D\}:Zero,One,Three:3,Six:6"";
        // Array of integers (an enum). hintString =
        $""\{!Variant.Type.String:D\}/\{!PropertyHint.File:D\}: * .png""; //
        Array of strings (file paths). hintString =
        ...
        // Two-dimensional array of textures. \[/csharp\] \[/codeblocks\]
        \[b\]Note:\[/b\] The trailing colon is required for properly detecting
        built-in types. *)
    ...
    (** Hints that a \[String\] property is a path to a file. Editing it will
        show a file dialog for picking the path for the file to be saved at. The
        dialog has access to the project''s directory. The hint string can be a
        set of filters with wildcards like \[code\]"" * .png, * .jpg""\[/code\].
        See also \[member FileDialog.filters\]. *)
    let _PROPERTY_HINT_SAVE_FILE = 27

    (** Hints that a \[String\] property is a path to a file. Editing it will
        show a file dialog for picking the path for the file to be saved at. The
        dialog has access to the entire filesystem. The hint string can be a set
        of filters with wildcards like \[code\]"" * .png, * .jpg""\[/code\]. See
        also \[member FileDialog.filters\]. *)
    ...
    (** Hints that a property is an \[Array\] with the stored type specified in
        the hint string. *)
    let _PROPERTY_HINT_ARRAY_TYPE = 31

    (** Hints that a property is a \[Dictionary\] with the stored types
        specified in the hint string. *)
    let _PROPERTY_HINT_DICTIONARY_TYPE = 38
    ...
    (** Hints that a property is an instance of a \[Node\]-derived type,
        optionally specified via the hint string (e.g.
        \[code\]""Node2D""\[/code\]). Editing it will show a dialog for picking
    ...
    (** Hints that a \[Callable\] property should be displayed as a clickable
        button. When the button is pressed, the callable is called. The hint
        string specifies the button text and optionally an icon from the
        \[code\]""EditorIcons""\[/code\] theme type. \[codeblock lang=text\]
        ""Click me!"" - A button with the text ""Click me!"" and the default
        ""Callable"" icon. ""Click me!,ColorRect"" - A button with the text
        ""Click me!"" and the ""ColorRect"" icon. \[/codeblock\]
        \[b\]Note:\[/b\] A \[Callable\] cannot be properly serialized and stored
        in a file, so it is recommended to use \[constant
        PROPERTY_USAGE_EDITOR\] instead of \[constant PROPERTY_USAGE_DEFAULT\].
    *)
    let _PROPERTY_HINT_TOOL_BUTTON = 39

    (** Hints that a property will be changed on its own after setting, such as
        \[member AudioStreamPlayer.playing\] or \[member
        GPUParticles3D.emitting\]. *)
    let _PROPERTY_HINT_ONESHOT = 40

    (** Represents the size of the \[enum PropertyHint\] enum. *)
    let _PROPERTY_HINT_MAX = 42
  end
  ...
  module PropertyUsageFlags = struct
    ...
    (** The property is a script variable. \[constant
        PROPERTY_USAGE_SCRIPT_VARIABLE\] can be used to distinguish between
        exported script variables from built-in variables (which don''t have
        this usage flag). By default, \[constant
        PROPERTY_USAGE_SCRIPT_VARIABLE\] is \[b\]not\[/b\] applied to variables
        that are created by overriding \[method Object._get_property_list\] in a
        script. *)
    let _PROPERTY_USAGE_SCRIPT_VARIABLE = 4096

    (** The property value of type \[Object\] will be stored even if its value
        is \[code\]null\[/code\]. *)
    ...
    (** If property has \[code\]nil\[/code\] as default value, its type will be
        \[Variant\]. *)
    ...
    (** When duplicating a resource with \[method Resource.duplicate\], and this
        flag is set on a property of that resource, the property should always
        be duplicated, regardless of the \[code\]subresources\[/code\] bool
        parameter. *)
    let _PROPERTY_USAGE_ALWAYS_DUPLICATE = 524288

    (** When duplicating a resource with \[method Resource.duplicate\], and this
        flag is set on a property of that resource, the property should never be
        duplicated, regardless of the \[code\]subresources\[/code\] bool
        parameter. *)
    ...
    (** The \[NodePath\] property will always be relative to the scene''s root.
        Mostly useful for local resources. *)
    ...
    (** Use when a resource is created on the fly, i.e. the getter will always
        return a different instance. \[ResourceSaver\] needs this information to
        properly save such resources. *)
    ...
    (** When this property is a \[Resource\] and base object is a \[Node\], a
        resource instance will be automatically created whenever the node is
  ...
  module MethodFlags = struct
    ...
    (** Used internally. Allows to not dump core virtual methods (such as
        \[method Object._notification\]) to the JSON API. *)
    let _METHOD_FLAG_OBJECT_CORE = 64

    (** Flag for a virtual method that is required. *)
    let _METHOD_FLAG_VIRTUAL_REQUIRED = 128
  ...
  module VariantType = struct
    ...
    let _TYPE_PACKED_VECTOR4_ARRAY = 38
