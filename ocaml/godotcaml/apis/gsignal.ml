...
let connect ?connect_flags f signal =
  let callable = Gcallable.of_fun signal.fn f in
  let result = BuiltinClass.Signal.connect callable (connect_flags |> Option.value ~default:0L) signal.raw in
  Result.ok_if_true Int64.(result = 0L) ~error:result

let emit : 'a -> 'a t -> unit =
  fun args signal ->
    BuiltinClass.Signal.emit (signal.marshal args) signal.raw
    
let emit2 : 'a0 -> 'a1 -> ('a0 * 'a1) t -> unit =
  fun x0 x1 signal ->
    BuiltinClass.Signal.emit (signal.marshal (x0, x1)) signal.raw

let emit3 : 'a0 -> 'a1 -> 'a2 -> ('a0 * 'a1 * 'a2) t -> unit =
  fun x0 x1 x2 signal ->
    BuiltinClass.Signal.emit (signal.marshal (x0, x1, x2)) signal.raw

let emit4 : 'a0 -> 'a1 -> 'a2 -> 'a3 -> ('a0 * 'a1 * 'a2 * 'a3) t -> unit =
  fun x0 x1 x2 x3 signal ->
    BuiltinClass.Signal.emit (signal.marshal (x0, x1, x2, x3)) signal.raw

let emit5 : 'a0 -> 'a1 -> 'a2 -> 'a3 -> 'a4 -> ('a0 * 'a1 * 'a2 * 'a3 * 'a4) t -> unit =
