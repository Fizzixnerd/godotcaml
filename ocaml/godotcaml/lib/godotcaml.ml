...
module TypedSuite : SUITE = struct
  ...
  let typedef_name ?prefix ?postfix name = sprintf "GDExtension%s%s%s" (prefix |> Option.value ~default:"") name (postfix |> Option.value ~default:"")

  let ptr_suite name (t : 'a structure typ) = 
    let def ptr_ty name t = typedef t (typedef_name ~prefix:ptr_ty ~postfix:"Ptr" name) in
...
module C (M: SUITE) = struct
  ...
  type variant_ptr
  let variant_ptr : variant_ptr M.ptr_suite = M.ptr_suite "Variant" (structure "gdovariant")
  type string_name_ptr
  let string_name_ptr : string_name_ptr M.ptr_suite = M.ptr_suite "StringName" (structure "gdostring_name")
  type string_ptr
  let string_ptr : string_ptr M.ptr_suite = M.ptr_suite "String" (structure "gdostring")
  type object_ptr
  let object_ptr : object_ptr M.ptr_suite = M.ptr_suite "Object" (structure "gdoobject")
  type type_ptr
  let type_ptr : type_ptr M.ptr_suite = M.ptr_suite "Type" (structure "gdotype")
  type method_bind_ptr
  let method_bind_ptr : method_bind_ptr M.ptr_suite = M.ptr_suite "MethodBind" (structure "gdomethod_bind")
  ...
  let type_constructor_from_variant_func = fn_suite (M.typedef_name "TypeConstructorFromVariantFunc") (type_ptr.uninit @-> variant_ptr.plain @-> returning void)
  let ptr_operator_evaluator = fn_suite (M.typedef_name "PtrOperatorEvaluator") (type_ptr.const @-> type_ptr.const @-> type_ptr.plain @-> returning void)
  let ptr_built_in_method = fn_suite (M.typedef_name "PtrBuiltInMethod") (type_ptr.plain @-> ptr type_ptr.const @-> type_ptr.plain @-> int @-> returning void)
  let ptr_constructor = fn_suite (M.typedef_name "PtrConstructor") (type_ptr.uninit @-> ptr type_ptr.const @-> returning void)
  let ptr_destructor = fn_suite (M.typedef_name "PtrDestructor") (type_ptr.plain @-> returning void)
  let ptr_setter = fn_suite (M.typedef_name "PtrSetter") (type_ptr.plain @-> type_ptr.const @-> returning void)
  let ptr_getter = fn_suite (M.typedef_name "PtrGetter") (type_ptr.const @-> type_ptr.plain @-> returning void)
  let ptr_indexed_setter = fn_suite (M.typedef_name "PtrIndexedSetter") (type_ptr.plain @-> gint @-> type_ptr.const @-> returning void)
  let ptr_indexed_getter = fn_suite (M.typedef_name "PtrIndexedGetter") (type_ptr.const @-> gint @-> type_ptr.plain @-> returning void)
  let ptr_keyed_setter = fn_suite (M.typedef_name "PtrKeyedSetter") (type_ptr.plain @-> type_ptr.const @-> type_ptr.const @-> returning void)
  let ptr_keyed_getter = fn_suite (M.typedef_name "PtrKeyedGetter") (type_ptr.const @-> type_ptr.const @-> type_ptr.plain @-> returning void)
  let ptr_keyed_checker = fn_suite (M.typedef_name "PtrKeyedChecker") (variant_ptr.const @-> variant_ptr.const @-> returning uint32_t)
  let ptr_utility_function = fn_suite (M.typedef_name "PtrUtilityFunction") (type_ptr.plain @-> ptr type_ptr.const @-> int @-> returning void)

  let class_constructor = fn_suite (M.typedef_name "ClassConstructor") (void @-> returning object_ptr.plain)

  let instance_binding_create_callback = fn_suite (M.typedef_name "InstanceBindingCreateCallback") (ptr void @-> ptr void @-> returning (ptr void))
  let instance_binding_free_callback = fn_suite (M.typedef_name "InstanceBindingFreeCallback") (ptr void @-> ptr void @-> ptr void @-> returning void)
  let instance_binding_reference_callback = fn_suite (M.typedef_name "InstanceBindingReferenceCallback") (ptr void @-> ptr void @-> gbool @-> returning gbool)

  module InstanceBindingCallbacks = struct
    type t

    let instance_binding_callbacks_struct : t structure typ = structure (M.typedef_name "InstanceBindingCallbacks")
    let create_callback_f = field instance_binding_callbacks_struct "create_callback" instance_binding_create_callback.typ
    let free_callback_f = field instance_binding_callbacks_struct "free_callback" instance_binding_free_callback.typ
    let reference_callback_f = field instance_binding_callbacks_struct "reference_callback" instance_binding_reference_callback.typ
    let () = seal instance_binding_callbacks_struct
    let s = typedef instance_binding_callbacks_struct (M.typedef_name "InstanceBindingCallbacks")
  end

  (* EXTENSION CLASSES *)

  type class_instance_ptr
  let class_instance_ptr : class_instance_ptr M.ptr_suite = M.ptr_suite "ClassInstance" (structure "gdoclass_instance_pointer")
  let class_set = fn_suite (M.typedef_name "ClassSet") (class_instance_ptr.plain @-> string_name_ptr.const @-> variant_ptr.const @-> returning gbool)
  let class_get = fn_suite (M.typedef_name "ClassGet") (class_instance_ptr.plain @-> string_name_ptr.const @-> variant_ptr.plain @-> returning gbool)
  let class_get_rid = fn_suite (M.typedef_name "ClassGetRID") (class_instance_ptr.plain @-> returning uint64_t)

  module PropertyInfo = struct
    type t

    let property_info_struct : t structure typ = structure (M.typedef_name "PropertyInfo")
    let type_f = field property_info_struct "type" variant_type
    let name_f = field property_info_struct "name" string_name_ptr.plain
    let class_name_f = field property_info_struct "class_name" string_name_ptr.plain
    let hint_f = field property_info_struct "hint" uint32_t
    let hint_string_f = field property_info_struct "hint_string" string_ptr.plain
  ...
  module MethodInfo = struct
    ...
    let method_info_struct : t structure typ = structure (M.typedef_name "MethodInfo")
    ...
    let default_argument_count_f = field method_info_struct "default_argument_count" uint32_t
    let default_arguments_f = field method_info_struct "default_arguments" variant_ptr.plain
    let () = seal method_info_struct
    let s = typedef method_info_struct (M.typedef_name "MethodInfo")
  end

  let class_get_property_list = fn_suite (M.typedef_name "ClassGetPropertyList") (class_instance_ptr.plain @-> ptr uint32_t @-> returning property_info_ptr.plain)
  let class_free_property_list = fn_suite (M.typedef_name "ClassFreePropertyList") (class_instance_ptr.plain @-> property_info_ptr.const @-> returning void)
